<x-app-layout>
	...
	<form method="POST" action="{{route('cliente.realidad.update', ['cliente' => $cliente->id, 'realidad' => $realidad->id])}}" enctype="multipart/form-data">
		@csrf
		@method('PUT')
		<div class="py-5">
			<div class="max-w-7xl mx-auto px-4 lg:px-8 bg-gray-100 py-10">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<input type="text" name="titulo" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" placeholder="Ejemplo: Términos y condiciones" value="{{ old('titulo', $realidad->titulo) }}" required>
					</div>
					<div>
						...
						<input class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" name="slug" id="slug"
							type="text" value="{{ old('slug', $realidad->slug) }}" placeholder="Ejemplo: terminos-y-condiciones" required>
					</div>
				</div>
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-8 mt-3">
					<div>
						...
						<input type="text" name="boton_texto" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" placeholder="Ejemplo: Misterio 3D Centro" value="{{ old('boton_texto', $realidad->boton_texto) }}" required>
					</div>
					<div>
						...
						<input class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" name="descripcion" id="descripcion"
							type="text" value="{{ old('descripcion', $realidad->descripcion) }}" placeholder="Ejemplo: Tómate la foto">
					</div>
				</div>
				<div class="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-8 mt-3">
					...
					<div>
						...
						<div id="preview-container"></div>
						<input type="file" id="glb" name="glb" multiple accept=".glb">
					</div>
					<div>
						...
						<div id="preview-container"></div>
						<input type="file" id="usdz" name="usdz" multiple accept=".usdz">
					</div>
				</div>
				<div class="mt-3">
					...
					<textarea class="input-border alx-editor" name="texto" id="texto"
						rows="5">{{ str_replace(["../../../../storage", "../https"], [url("storage"), "https"], old('texto', $realidad->texto)) }}</textarea>
				</div>
				<div class="mt-10">
