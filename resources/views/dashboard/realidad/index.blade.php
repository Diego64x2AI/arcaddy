<x-app-layout>
	<x-slot name="header">
		<div class="flex items-center">
			...
			<div class="ml-auto">
				<a href="{{ route('cliente.realidad.create', ['cliente' => $cliente->id]) }}" class="rounded-full bg-pink-600 text-white px-5 py-2 block">
					Crear
				</a>
			</div>
		</div>
						...
						<tbody>
							...
							<tr>
								...
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									<a href="{{ asset('storage/qrcodes_ar/'.$link->id.'.png?'.time()) }}" download="qr-{{ $link->slug }}" target="_blank" title="Código QR">
										<img src="{{ asset('storage/qrcodes_ar/'.$link->id.'.png?'.time()) }}" alt="Código QR" class="w-20 h-auto inline-block">
									</a>
								</td>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									{{ $link->titulo }}
								...
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									<a href="{{ route('cliente_ar', ['slug' => $link->cliente->slug, 'slug2' => $link->slug]) }}" class="text-black" target="_blank"><i class="fas fa-link"></i></a>
									<a href="{{ route('cliente.realidad.edit', ['cliente' => $cliente->id, 'realidad' => $link->id]) }}" class="text-black"><i class="fas fa-edit"></i></a>
									<form action="{{ route('cliente.realidad.destroy', ['cliente' => $cliente->id, 'realidad' => $link->id]) }}" method="POST" class="inline delete-form">
										@csrf
										@method('DELETE')
