<x-app-layout>
	...
	<form method="POST" action="{{route('cliente.qrlinks.store', ['cliente' => $cliente->id])}}" enctype="multipart/form-data">
		@csrf
		<div class="py-5">
			<div class="max-w-7xl mx-auto px-4 lg:px-8 bg-gray-100 py-10">
				...
				<div id="banners" class="bg-white p-3 mt-3">
					...
					<div class="flex flex-row items-center font-bold">
						...
						<div class="ml-5">
							<span class="hidden md:inline-block mr-2">Módulo activo </span><input type="checkbox" name="banners-activo" value="on" @if($cliente->id !== NULL && $cliente->secciones()->where('seccion', 'banners')->first()->activa) checked @endif>
						</div>
					</div>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<input type="text" name="boton_texto" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" placeholder="Ejemplo: Misterio 3D Centro" value="{{ old('boton_texto') }}">
					</div>
					<div>
						...
						<input class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" name="boton_link" id="boton_link"
							type="url" value="{{ old('boton_link') }}" placeholder="Ejemplo: https://gioogle.com">
					</div>
				</div>
				<div class="mt-10">
