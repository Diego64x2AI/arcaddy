<div id="blog" class="bg-white p-3 mt-3">
	...
	<div class="flex flex-row items-center font-bold">
		<div class="text-xl md:text-3xl truncate mr-5 grow">
			<input class="shadow appearance-none border w-full py-2 px-3 text-gray-700" name="titulos[blog]" type="text"
			value="{{ ($cliente->id !== NULL && $cliente->secciones()->where('seccion', 'blog')->first()->titulo !== NULL) ? $cliente->secciones()->where('seccion', 'blog')->first()->titulo : 'Blog / Lifestyle' }}">
		</div>
		<div class="ml-auto">
			<span class="hidden md:inline-block">Mostrar título </span><input type="checkbox" name="blog-activo2" value="on" @if($cliente->id !== NULL && $cliente->secciones()->where('seccion', 'blog')->first()->mostrar_titulo) checked @endif>
		</div>
		<div class="ml-5">
