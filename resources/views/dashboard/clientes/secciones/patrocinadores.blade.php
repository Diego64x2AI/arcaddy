<div id="patrocinadores" class="bg-white p-3 mt-3">
	...
	<div class="flex flex-row items-center font-bold">
		<div class="text-xl md:text-3xl truncate mr-5 grow">
			<input class="shadow appearance-none border w-full py-2 px-3 text-gray-700" name="titulos[patrocinadores]" type="text"
			value="{{ ($cliente->id !== NULL && $cliente->secciones()->where('seccion', 'patrocinadores')->first()->titulo !== NULL) ? $cliente->secciones()->where('seccion', 'patrocinadores')->first()->titulo : 'Patrocinadores' }}">
		</div>
		<div class="ml-auto">
			<span class="hidden md:inline-block">Mostrar título </span><input type="checkbox" name="patrocinadores-activo2" value="on" @if($cliente->id !== NULL && $cliente->secciones()->where('seccion', 'patrocinadores')->first()->mostrar_titulo) checked @endif>
		</div>
		<div class="ml-5">
			<span class="hidden md:inline-block">Módulo activo </span><input type="checkbox" name="patrocinadores-activo" value="on" @if($cliente->id !== NULL && $cliente->secciones()->where('seccion', 'patrocinadores')->first()->activa) checked @endif>
		</div>
			...
			<div class="w-2/4 md:w-1/4 float-left bg-white hover:bg-gray-100 hover:shadow fotometria-box group">
				<div class="p-3">
					...
					<div class="mb-2">
						...
						<input class="input-underline" name="patrocinadores_titulo[]" value="{{ $patrocinador->titulo }}" type="text" >
					</div>
					<div class="mb-2">
						<label class="block tracking-wide text-gray-700 text-base font-bold mb-1">
							LINK:
