<x-app-layout>
	...
	<form method="POST" action="{{route('cliente.sucursales.update', ['cliente' => $cliente->id, 'sucursale' => $sucursal->id])}}"
		...
		<div class="py-5">
			<div class="max-w-7xl mx-auto px-4 lg:px-8 bg-gray-100 py-10">
				...
				<div class="text-center mb-10">
					<a href="{{ asset('storage/sucursales/'.$sucursal->id.'.png?'.time()) }}" download="qr-sucursal-{{ $sucursal->id }}" class="inline-block" target="_blank" title="Código QR">
						<img src="{{ asset('storage/sucursales/'.$sucursal->id.'.png?'.time()) }}" alt="{{ $sucursal->nombre }}" class="w-28 h-auto inline-block">
					</a>
				</div>
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<input type="text" name="nombre" id="nombre" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700"
							value="{{ old('nombre', $sucursal->nombre) }}" placeholder="Ejemplo: Enrique ladrón de guevara" required>
					</div>
					<div>
					...
					<div>
						...
						<input type="text" name="ciudad" id="ciudad" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700"
							value="{{ old('ciudad', $sucursal->ciudad) }}" placeholder="Ejemplo: Zapopan" required>
					</div>
					<div>
						...
						<input type="text" name="horario" id="horario" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700"
							value="{{ old('horario', $sucursal->horario) }}" placeholder="Ejemplo: 9am a 6pm">
					</div>
					<div>
						...
						<input type="text" name="link_titulo" id="link_titulo"
							...
							value="{{ old('link_titulo', $sucursal->link_titulo) }}">
					</div>
					<div>
					...
					<div>
						...
						<input type="text" name="lat" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700"
							value="{{ old('lat', $sucursal->lat) }}" required>
					</div>
					<div>
						...
						<input type="text" name="lng" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700"
							value="{{ old('lng', $sucursal->lng) }}" required>
					</div>
					<div class="col-span-2">
						<div class="text-xs">
