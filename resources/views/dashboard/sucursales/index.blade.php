<x-app-layout>
	<x-slot name="header">
		<div class="flex items-center">
			...
			<div class="ml-auto">
				<a href="{{ route('cliente.sucursales.create', ['cliente' => $cliente->id]) }}"
							...
							<div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
								@foreach ($sucursales as $sucursal)
									<div class="flex flex-row items-center gap-3 text-sm border-b py-5">
										<div>
											<img src="{{ ($cliente->sucursales_pin !== NULL) ? asset('storage/' . $cliente->sucursales_pin) : asset('images/sucursal-pin.png') }}"
												alt="{{ $sucursal->nombre }}" class="inline-block w-16 h-auto">
										</div>
										<div class="text-center grow">
											<div class="font-bold line-clamp-2 text-basex leading-4">{{ $sucursal->nombre }}
											</div>
											<div>{{ $sucursal->ciudad }}</div>
										</div>
										<div>
											<div class="text-center">
												<span class="text-pink-600 font-bold">{{ $sucursal->lecturas }}</span>
												Lecturas
											</div>
											<div class="grid grid-cols-3 gap-3">
												<div class="text-center">
													<a href="{{ asset('storage/sucursales/' . $sucursal->id . '.png?' . time()) }}"
														download="qr-sucursal-{{ $sucursal->id }}" class="inline-block"
														target="_blank" title="Código QR"><i class="fa fa-qrcode"></i></a>
												</div>
												<div class="text-center">
													<a href="{{ route('cliente.sucursales.edit', ['cliente' => $sucursal->cliente_id, 'sucursale' => $sucursal->id]) }}"
														class="inline-block" title="Editar"><i class="fa fa-edit"></i></a>
												</div>
												<div class="text-center">
													<form
														action="{{ route('cliente.sucursales.destroy', ['cliente' => $sucursal->cliente_id, 'sucursale' => $sucursal->id]) }}"
														method="POST" class="delete-form inline-block m-0 b-0">
														@csrf
														@method('DELETE')
														<button
															href="{{ route('cliente.sucursales.destroy', ['cliente' => $sucursal->cliente_id, 'sucursale' => $sucursal->id]) }}"
															type="button" wire:click="$emit('confirmDelete')">
