<x-app-layout>
	...
	<form id="image-form" method="POST" action="{{route('grupos.update', ['grupo' => $grupo->id])}}" enctype="multipart/form-data">
		@csrf
		@method('PUT')
		<div class="py-5">
			<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 bg-gray-100 py-10">
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<input type="text" name="nombre" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('nombre', $grupo->nombre) }}" required>
					</div>
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="slug">
							Logo
						...
						<div id="preview-container">
							@if ($grupo->logo !== NULL)
								<img src="{{ asset('storage/'.$grupo->logo) }}" class="w-20 h-auto">
							@endif
						</div>
				...
				<div id="miembros" class="grid grid-cols-2 lg:grid-cols-6 justify-center items-center gap-3 mt-10">
					...
					<div class="text-center">
						<div>
							<img src="{{ asset('storage/'.$miembro->cliente->logo) }}" class="w-auto h-7 inline-block" alt="{{ $miembro->cliente->nombre }}">
						</div>
						<div class="text-sm">
						...
						<input type="hidden" name="miembro[]" value="{{ $miembro->cliente->id }}" >
					</div>
					@endforeach
				</div>
				<div class="mt-10">
