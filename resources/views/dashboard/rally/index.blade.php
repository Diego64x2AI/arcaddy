<x-app-layout>
	<x-slot name="header">
		<div class="flex items-center">
			...
			<div class="ml-auto">
				<a href="{{ route('cliente.rally.create', ['cliente' => $cliente->id]) }}" class="rounded-full bg-pink-600 text-white px-5 py-2 block">
					Crear
				</a>
			</div>
		</div>
					...
					<table id="usuarios" class="w-full rounded-lg overflow-hidden sm:shadow-lg flex-nowrap !my-5">
						<thead class="text-white">
							<tr class="bg-teal-400">
								...
								<th class="p-3 !text-center">Geo Oculto</th>
								<th class="p-3 !text-center">Activo</th>
								<th class="p-3 !text-center">Ubicaciones</th>
						...
						<tbody>
							...
							<tr>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">{{ $rally->id }}</td>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									@if($rally->banner !== NULL)
									<img src="{{ asset('storage/'.$rally->banner) }}" alt="{{ $rally->titulo }}" class="w-20 h-auto object-cover inline-block">
									@endif
								</td>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									{{ $rally->titulo }}
								...
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									{{ $rally->geo_oculto ? 'Si' : 'No' }}
								</td>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									{{ $rally->activo ? 'Si' : 'No' }}
								</td>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									{{ $rally->ubicaciones_count }}
								</td>
								<td class="border-grey-light border hover:bg-gray-100 p-3 text-center">
									<a href="{{ route('cliente.rally.markers', ['cliente' => $cliente->id, 'rally' => $rally->id]) }}" class="text-black"><i class="fas fa-map-marker-alt"></i></a>
									<a href="{{ route('cliente.rally.edit', ['cliente' => $cliente->id, 'rally' => $rally->id]) }}" class="text-black"><i class="fas fa-edit"></i></a>
									<form action="{{ route('cliente.rally.destroy', ['cliente' => $cliente->id, 'rally' => $rally->id]) }}" method="POST" class="inline delete-form">
										@csrf
										@method('DELETE')
