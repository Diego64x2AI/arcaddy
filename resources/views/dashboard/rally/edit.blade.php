<x-app-layout>
	...
	<form method="POST" action="{{route('cliente.rally.update', ['cliente' => $cliente->id, 'rally' => $rally->id])}}" enctype="multipart/form-data">
		@csrf
		@method('PUT')
		<div class="py-5">
			<div class="max-w-7xl mx-auto px-4 lg:px-8 bg-gray-100 py-10">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<input type="text" name="titulo" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('titulo', $rally->titulo) }}" required>
					</div>
					<div>
					...
					<div class="font-bold">
						<label for="geo_oculto" class="flex items-center cursor-pointer">
							<div class="relative mr-5">
								<input id="geo_oculto" name="geo_oculto" type="checkbox" class="sr-only" @checked(old('geo_oculto', $rally->geo_oculto)) />
					...
					<div class="font-bold">
						<label for="activo" class="flex items-center cursor-pointer">
							<div class="relative mr-5">
								<input id="activo" name="activo" type="checkbox" class="sr-only" @checked(old('activo', $rally->activo)) />
								<div class="w-10 h-4 bg-gray-400 rounded-full shadow-inner"></div>
								<div class="dot absolute w-6 h-6 bg-white rounded-full shadow -left-1 -top-1 transition"></div>
							</div>
							Activo
