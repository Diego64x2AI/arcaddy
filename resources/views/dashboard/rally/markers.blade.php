<x-app-layout>
	<x-slot name="header">
		<div class="flex items-center">
			...
			<div class="ml-auto">
				<a href="{{ route('cliente.rally.index', ['cliente' => $cliente->id]) }}"
			...
			<div class="flex flex-col">
				<div class="grow">
							...
							@foreach ($rally->ubicaciones as $ubicacion)
								<div class="bg-white shadow p-5 flex flex-col">
									<div class="text-center">
										<img src="{{ asset('storage/' . $ubicacion->marker) }}" alt="{{ $ubicacion->titulo }}"
											class="w-auto h-16 inline-block">
									</div>
									...
									<div class="flex flex-row items-center justify-center gap-2 mt-auto">
										<a href="javascript:void(0);" data-lat="{{ $ubicacion->lat }}"
											data-lng="{{ $ubicacion->lng }}" class="distancia-actual"
											title="Distancia actual"><i class="fas fa-ruler-combined"></i></a>
										<a href="{{ route('cliente.rally.markers.edit', ['cliente' => $cliente->id, 'ubicacion' => $ubicacion->id, 'rally' => $rally->id]) }}"
											title="Editar"><i class="fa fa-edit"></i></a>
										<form class="delete-form"
											action="{{ route('cliente.rally.markers.destroy', ['cliente' => $cliente->id, 'ubicacion' => $ubicacion->id, 'rally' => $rally->id]) }}"
											method="POST">
											@csrf
											@method('DELETE')
											<button type="submit" title="Eliminar" class="text-red-600"><i
					...
					<div class="flex flex-col justify-center items-center my-10">
						<a href="{{ route('cliente.rally.markers.create', ['cliente' => $cliente->id, 'rally' => $rally->id]) }}"
							class="rounded-full bg-pink-600 text-white text-center overflow-hidden flex flex-col justify-center items-center p-5 text-xs lg:text-base w-[6rem] h-[6rem] lg:w-[8rem] lg:h-[8rem]">
