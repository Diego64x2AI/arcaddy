		...
		<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
			<div>
				<div class="text-center">
					<img src="{{ asset('storage/' . $cliente->logo) }}" alt="{{ $cliente->cliente }}"
						class="inline-block w-full h-auto max-w-xs">
				</div>
				...
				<div>
					@if ($errors->any())
						<div class="mb-5">
							<div class="relative w-full p-4 text-white bg-yellow-400 rounded-lg">{{ $errors->first() }}
							</div>
						</div>
					@endif
					@if (session('success'))
						<div class="mb-5">
							<div class="relative w-full p-4 text-white bg-lime-500 rounded-lg">{{ session('success') }}
							</div>
						</div>
					@endif
					<div class="grid grid-cols-1 lg:grid-cols-3 gap-10 xl:gap-16 mt-10">
									...
									@foreach ($marcos as $galeria)
										<div>
											<img src="{{ asset('storage/' . $galeria->archivo) }}"
												alt="{{ $galeria->titulo }}" class="w-full h-auto">
										</div>
									@endforeach
								</div>
							</div>
						...
						<div class="lg:col-span-2">
								...
								@foreach ($visitas as $visita)
									<div class="grid grid-cols-4 items-center font-semibold text-sm mt-5">
										<div class="font-bold truncate col-span-3">
											<a href="{{ $visita->url }}" class="underline"
												target="_blank">{{ $visita->url }}</a>
										</div>
										<div class="ml-auto">
							...
							<div class="bg-white w-full p-5 mt-5 xl:mt-10 border border-white rounded-3xl">
								...
								<div
									class="grid-cols-1 lg:grid-cols-6 items-center font-semibold text-sm mt-5 hidden lg:grid">
									<div
										class="font-bold text-xl text-pink-600 truncate lg:col-span-3 text-center lg:text-left">
										Usuario
									</div>
								...
								@foreach ($top_users as $visita)
									...
									@endphp
									<div
										class="grid grid-cols-3 gap-5 lg:grid-cols-6 items-center font-semibold text-sm mt-5">
										...
										<div class="text-center">
											<div>{{ $redencioens_stats['ganados'] }} / {{ $redencioens_stats['canjeados'] }}
											</div>
									...
									<div class="grid grid-cols-4 gap-5 items-start border-b border-gray-300 py-3">
										<div class="col-span-1 font-bold text-xl">
											<img src="{{ asset('storage/' . $producto->imagenes[0]->archivo) }}"
												alt="{{ $producto->nombre }}" class="w-full h-auto shadow border">
										</div>
										<div class="col-span-3 font-semibold text-xl">
											<div>{{ $producto->nombre }}</div>
											<div class="text-sm text-gray-500">{{ $producto->descripcion }}</div>
											<div class="mt-4 text-sm">
												<span class="text-pink-600">Canjes totales:</span>
												{{ $producto->canjeados->count() }}
											</div>
								...
								@foreach ($beneficios as $producto)
									<div class="grid grid-cols-4 gap-5 items-start border-b border-gray-300 py-3">
										...
										<div class="col-span-3 font-semibold text-xl">
											...
											<div class="text-sm text-gray-500">{{ $producto->descripcion }}</div>
											<div class="mt-4 text-sm">
												<span class="text-pink-600">Stock actual:</span>
												{{ ($producto->cantidad === -1) ? 'Ilimitado' : $producto->cantidad }}
											</div>
											<div class="mt-0 text-sm">
												<span class="text-pink-600">Canjes totales:</span>
												{{ $producto->beneficios()->where('canjeado', 1)->count() }}
											</div>
							...
							<div class="grid grid-cols-1 lg:grid-cols-2 mt-5 xl:mt-10 gap-10 xl:gap-16">
								<div class="bg-white w-full p-5 border border-white rounded-3xl">
									...
									@foreach ($qrlinks as $link)
										<div class="flex flex-row items-center font-semibold text-sm mt-5">
											<div class="font-bold">
												<a href="{{ route('cliente_seccion', ['slug' => $link->cliente->slug, 'slug2' => $link->slug]) }}"
													class="underline" target="_blank">{{ $link->titulo }}</a>
											</div>
											<div class="ml-auto">
								...
								<div class="bg-white w-full p-5 border border-white rounded-3xl">
									...
									@foreach ($realidades as $link)
										<div class="flex flex-row items-center font-semibold text-sm mt-5">
											<div class="font-bold">
												<a href="{{ route('cliente_ar', ['slug' => $link->cliente->slug, 'slug2' => $link->slug]) }}"
													class="underline" target="_blank">{{ $link->titulo }}</a>
											</div>
											<div class="ml-auto">
												<span class="text-pink-600">{{ $link->lecturas }}</span> lecturas
					...
					@if ($quiz !== NULL)
						...
						<h3 class="text-xl font-extrabold my-5">{{ $quiz->nombre }} <span
								class="text-pink-600">({{ $quiz_totales }} respuestas)</span></h3>
						<div id="pdf-container" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
							@foreach ($quiz_respuestas as $respuesta)
								<div class="bg-gray-200 px-5 py-8 shadow">
									<div class="font-bold text-end mb-8">
										{{ str_replace(['open', 'level', 'option', 'like', 'multi', 'versus'], ['Abierta', 'Level Satisfaction', 'Opción', 'Like or Not', 'Multiple', 'VS'], $respuesta['pregunta']->tipo) }}
									</div>
									...
									@elseif ($respuesta['pregunta']->tipo == 'like')
										<div class="flex flex-col gap-3">
											<div>
												<img src="{{ asset('storage/' . $respuesta['pregunta']->archivo) }}"
													alt="{{ $respuesta['pregunta']->pregunta }}"
													class="object-cover w-full h-auto border border-secondary shadow rounded-3xl">
											</div>
											<dib class="grid grid-cols-2 items-center gap-1 mt-3 font-semibold">
												<div class="flex flex-row justify-center">
													<a href="javascript:void(0);"
														class="rounded-full cursor-default border-0 border-white like-click bg-pink-600 text-white text-center overflow-hidden flex flex-col justify-center text-3xl items-center p-4">
														<i class="far fa-thumbs-up"></i>
													</a>
												</div>
												<div class="flex flex-row justify-center">
													<a href="javascript:void(0);"
														class="rounded-full cursor-default border-0 border-white dislike-click bg-pink-600 text-white text-center overflow-hidden flex flex-col justify-center text-3xl items-center p-4">
									...
									@elseif ($respuesta['pregunta']->tipo == 'level')
										<div class="flex flex-col gap-3">
											...
											@endif
											<div>
												<div data-value="{{ round($respuesta['promedio'], 2) }}" class="mt-5 slider-level">
												</div>
											</div>
											...
											<dib class="grid grid-cols-2 items-center gap-3 font-semibold">
												<div class="flex flex-row justify-center">
													<div class="mb-3 relative">
														<img src="{{ asset('storage/' . $respuesta['pregunta']->respuestas->where('tipo', 'versus1')->first()?->archivo) }}"
															class="object-cover w-full h-auto border border-secondary shadow rounded-3xl">
													</div>
												</div>
												<div class="flex flex-row justify-center">
													<div class="mb-3 relative">
														<img src="{{ asset('storage/' . $respuesta['pregunta']->respuestas->where('tipo', 'versus2')->first()?->archivo) }}"
															class="object-cover w-full h-auto border border-secondary shadow rounded-3xl">
													</div>
												</div>
												<div class="flex flex-col items-center text-center justify-center">
										...
										<div class="">
											...
											<div>
												<canvas class="pie-chart"
													data-json="{{ json_encode($respuesta['dataset'] ?? []) }}"></canvas>
											</div>
										</div>
									@endif
								</div>
										...
										@foreach ($game['scores'] as $score)
											...
											<div class="flex flex-row items-center mb-2">
												...
												<div class="flex flex-row grow ml-2 items-center px-3 py-2 rounded-3xl" {!! $style !!}>
													{{--<div><img src="{{ asset('images/Imagen 73.jpg') }}"
															class="w-10 h-10 rounded-full" alt="Juan Carlos Perez"></div>--}}
													<div class="grow text-left ml-2 font-semibold text-xs md:text-normal">
														{{ $score->user->name }}</div>
													<div class="ml-auto font-extrabold text-xl">{{ $score->tiempo }}
														{{ __('arcaddy.seconds') }}</div>
												</div>
											</div>
										@endforeach
									</div>
