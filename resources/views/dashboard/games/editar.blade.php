<x-app-layout>
	...
	<form id="image-form" method="POST" action="{{ route('games.update', ['game' => $juego->id]) }}" enctype="multipart/form-data">
		@csrf
		@method('PUT')
		<div class="py-5">
			<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 bg-gray-100 py-10">
				...
				@endif
				@php
				$path = ($juego->categoria->slug === 'shuffle-puzzle') ? '' : 'clientes/games/memory/';
				@endphp
				<div class="w-full mx-auto mb-10 grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-3 lg:gap-6 items-center place-items-center justify-center">
					@foreach($cartas as $carta)
					<div class="relative">
						<img src="{{ asset('storage/'.$path.$carta->imagen) }}" alt="" class="w-full h-auto">
						<div class="carta-delete" data-id="{{$carta->id}}" data-aviso="front">X</div>
					</div>
					@endforeach
					@foreach($cartasBack as $carta)
					<div class="relative">
						<img src="{{ asset('storage/'.$path.$carta->imagen) }}" alt="" class="w-full h-auto">
						<div class="carta-delete" data-id="{{$carta->id}}" data-aviso="front">X</div>
					</div>
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<select name="cliente_id" class="shadow appearance-none border-0 w-full !py-2 !px-3 text-gray-700 input-underline" required>
							...
							<option value="{{$cliente->id}}" @if(intval(old('cliente_id', $juego->cliente_id)) === $cliente->id) selected @endif>{{$cliente->cliente}}</option>
							@endforeach
						</select>
					</div>
					<div>
						...
						<select name="estatus" class="shadow appearance-none border-0 w-full !py-2 !px-3 text-gray-700 input-underline" required>
							<option value="1" @if(intval(old('estatus', $juego->activo)) === 1) selected @endif>Si</option>
							<option value="0" @if(intval(old('estatus', $juego->activo)) === 0) selected @endif>No</option>
						</select>
					</div>
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="slug">
						...
						<input type="text" name="nombre" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('nombre', $juego->nombre) }}" required>
					</div>
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="slug">
							Descripción
						</label>
						<input type="text" name="descripcion" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('descripcion', $juego->descripcion) }}" required>
					</div>
					@if ($juego->categoria->slug === 'shuffle-puzzle')
					<div>
						...
						<select id="dificultad" name="dificultad" class="shadow appearance-none border-0 w-full !py-2 !px-3 text-gray-700 input-underline" required>
							<option value="3x3" @if(old('dificultad', $juego->dificultad) === '3x3') selected @endif>3x3</option>
							<option value="4x4" @if(old('dificultad', $juego->dificultad) === '4x4') selected @endif>4x4</option>
							<option value="5x5" @if(old('dificultad', $juego->dificultad) === '5x5') selected @endif>5x5</option>
						</select>
					</div>
					<div>
					...
					@endif
					@if ($juego->categoria->slug === 'memory')
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="slug">
							Cartas
						</label>
