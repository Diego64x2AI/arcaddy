<x-app-layout>
	...
	<form method="POST" action="{{route('cliente.qrexperiencias.update', ['cliente' => $cliente->id, 'qrexperiencia' => $qrexperiencia->id])}}" enctype="multipart/form-data">
		@csrf
		@method('PUT')
			...
			<div class="max-w-7xl mx-auto px-4 lg:px-8 bg-gray-100 py-10">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-8">
					<div>
						...
						<input type="text" name="titulo" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('titulo', $qrexperiencia->titulo) }}" required>
					</div>
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="url">
							URL
						</label>
						<input type="url" name="url" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('url', $qrexperiencia->url) }}" required>
					</div>
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="lat">
							Latitud
						</label>
						<input type="text" name="lat" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('lat', $qrexperiencia->lat) }}" required>
					</div>
					<div>
						<label class="block tracking-wide text-gray-700 text-xl font-bold mb-2" for="lng">
							Longitud
						</label>
						<input type="text" name="lng" class="shadow appearance-none border-0 w-full py-2 px-3 text-gray-700" value="{{ old('lng', $qrexperiencia->lng) }}" required>
					</div>
